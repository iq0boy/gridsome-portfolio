    ...
    <div class="container sm:pxi-0 mx-auto overflow-x-hidden pt-24 pb-12">
      <div class="px-4 pt-8">
        <section class="mb-4 border-b">
          <h1
            class="font-medium leading-none mt-0 mb-4 md:mb-8 flex flex-col md:flex-row items-baseline"
          >
          ...
          <ul class="flex flex-wrap mb-5">
            <li
              class="skill-tag"
              v-for="(skill, index) in $page.project.skills"
              :key="index"
            >
              <g-link :to="skill.path">
                <span class="skill-label">{{ skill.title }}</span
                ><span
                  class="ml-2 mr-2 text-gray-400"
                  v-if="index + 1 < $page.project.skills.length"
                  >-</span
                >
              </g-link>
            </li>
          ...
          <div
            ...
            v-html="$page.project.shortDescription"
          ></div>
        </section>
      </div>
      ...
      <div class="px-4 mt-10">
        <blockquote>
          <p class="font-bold">
            Status :
            {{
              $page.project.status.charAt(0).toUpperCase() +
              $page.project.status.slice(1)
            }}
          </p>
      ...
      <div class="px-4 mt-4 mb-2 relative text-xl h-12">
        <div v-if="$page.next" class="absolute left-0 ml-2">
          <g-link
            :to="$page.next.path"
            class="flex justify-between items-center cursor-pointer nav-arrow"
          >
